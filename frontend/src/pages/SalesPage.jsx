...
const initialForm = {
  senderName: '', senderDocument: '', senderCountryCode: 'PA', senderTitle: '', senderIsMinor: false, senderFirstName: '', senderMiddleName: '', senderFirstLastName: '', senderSecondLastName: '', senderPhone: '', senderSecondaryPhone: '', senderEmail: '', senderProvince: '', senderCity: '', senderPostalCode: '', senderStreet: '', senderHouseNumber: '', senderAddress: '', senderFax: '',
  recipientName: '', recipientTitle: '', recipientFirstName: '', recipientMiddleName: '', recipientFirstLastName: '', recipientSecondLastName: '', recipientPhone: '', recipientSecondaryPhone: '', recipientEmail: '', recipientProvince: '', recipientCity: '', recipientPostalCode: '', recipientStreet: '', recipientHouseNumber: '', recipientAddress: '', recipientFax: '',
  shipmentScope: 'national', postalServiceId: '', destinationId: '', weightKg: '', supplementaryServiceIds: [], paymentMethodId: '',
}
...
const recipientFullName = (value) => [value.recipientFirstName, value.recipientMiddleName, value.recipientFirstLastName, value.recipientSecondLastName].filter((part) => part?.trim()).join(' ')
...
function printInvoice(sale) {
  ...
  const receiptFormat = shipment.postalFormCode ? ` · FORMATO ${safe(shipment.postalFormCode)}` : ''
  printDocument(sale.invoiceNumber, `<main class="receipt"><header class="center"><div class="receipt-brand"><span>CORREOS</span> PANAMÁ</div><div>Dirección Nacional de Correos y Telégrafos</div><div class="receipt-title">RECIBO DE VENTA${receiptFormat}</div></header><div class="rule"></div><div class="line"><span>RECIBO:</span><strong>${safe(sale.invoiceNumber)}</strong></div><div class="line"><span>FECHA:</span><span>${safe(new Date(sale.createdAtUtc).toLocaleString('es-PA'))}</span></div><div class="line"><span>CAJA:</span><span>${safe(receipt.terminalCode)}</span></div><div class="line"><span>OFICINISTA:</span><span>${safe(receipt.cashier)}</span></div><div class="line"><span>ESTAFETA:</span><strong>${safe(receipt.officeCode)} · ${safe(receipt.officeName)}</strong></div><div class="rule"></div><div class="section">REMITENTE</div><div>${safe(shipment.senderName)}</div><div>Cédula/Pasaporte: ${safe(shipment.senderDocument || 'N/D')}</div><div>${safe(shipment.senderAddress)}</div><div>Tel.: ${safe(shipment.senderPhone || 'N/D')}</div><div class="rule"></div><div class="section">ENVÍO POSTAL</div><div class="detail"><strong>DESTINATARIO:</strong> ${safe(shipment.recipientName)}<br><strong>DIRECCIÓN:</strong> ${safe(shipment.recipientAddress)}<br><strong>DESTINO:</strong> ${safe(shipment.destination)}<br><strong>SERVICIO:</strong> ${safe(shipment.postalService)}<br><strong>PESO:</strong> ${(shipment.weightGrams / 1000).toFixed(3)} kg<br><strong>S10:</strong> ${safe(shipment.trackingNumber)}</div><table><thead><tr><th>CANT. DESCRIPCIÓN</th><th>IMPORTE</th></tr></thead><tbody>${sale.lines.map((line) => `<tr><td>${line.quantity} ${safe(line.description)}</td><td>B/.${Number(line.total).toFixed(2)}</td></tr>`).join('')}</tbody></table><div class="rule"></div><div class="line grand"><span>TOTAL:</span><strong>B/.${Number(sale.total).toFixed(2)}</strong></div>${sale.payments.map((payment) => `<div class="line"><span>${safe(payment.paymentMethod)}:</span><span>B/.${Number(payment.amount).toFixed(2)}</span></div>`).join('')}<div class="rule"></div><div class="center"><strong>DOCUMENTO NO FISCAL</strong><p>Conserve este recibo para cualquier reclamo.</p><div class="barcode">${shipment.barcodeSvg}</div><strong>${safe(shipment.trackingNumber)}</strong><p>Tu correo seguro y económico</p></div></main>`, '80mm auto')
}

function printLabel(sale) {
  const shipment = sale.shipment
  const officeCode = sale.receipt?.officeCode || 'N/D'
  const countryCode = shipment.trackingNumber.slice(-2)
  const serviceMark = shipment.trackingNumber.slice(0, 1)
  const formBadge = shipment.postalFormCode ? `<strong>${safe(shipment.postalFormCode)}</strong>` : ''
  const labelStyles = '<style>.label-header .format{display:none}.label .s10-card{border-bottom:1px solid #b8c8d8;padding:2px 0 8px}.label .s10-head{display:flex;align-items:center;gap:7px}.label .s10-mark{font:900 26px/1 Arial,sans-serif;color:#111}.label .s10-brand{font:900 10px/1.05 Arial,sans-serif;color:#003b70}.label .s10-brand span{display:block;color:#ef7d00;font-size:7px;letter-spacing:.7px;margin-top:2px}.label .s10-meta{display:flex;align-items:center;gap:9px;margin-left:auto;font-size:9px;color:#526579}.label .s10-meta strong{color:#003b70;font-size:14px;letter-spacing:.5px}.label .s10-barcode{margin-top:5px}.label .s10-barcode svg{display:block;width:100%;height:auto;max-height:1.08in}</style>'
  const labelBody = `<main class="label"><header class="label-header"><div class="brand-lockup"><span class="brand-icon">✉</span><div><strong>CORREOS</strong><b>PANAMÁ</b></div></div></header><section class="s10-card"><div class="s10-head"><div class="s10-mark">${safe(serviceMark)}</div><div class="s10-brand">CORREOS<span>PANAMÁ</span></div><div class="s10-meta"><span>Estafeta ${safe(officeCode)} - ${safe(countryCode)}</span>${formBadge}</div></div><div class="s10-barcode">${shipment.barcodeSvg}</div></section><section class="address recipient"><div class="caption">DESTINATARIO / PARA</div><h2>${safe(shipment.recipientName)}</h2><p>${safe(shipment.recipientAddress)}<br><strong>${safe(shipment.destination)}</strong>${shipment.recipientPhone ? `<br>Tel. ${safe(shipment.recipientPhone)}` : ''}${shipment.recipientEmail ? `<br>${safe(shipment.recipientEmail)}` : ''}</p></section><section class="address"><div class="caption">REMITENTE / DE</div><h2>${safe(shipment.senderName)}</h2><p>${safe(shipment.senderAddress)}${shipment.senderPhone ? `<br>Tel. ${safe(shipment.senderPhone)}` : ''}</p></section><div class="service-strip"><span><strong>SERVICIO</strong><br>${safe(shipment.postalService)}</span><span><strong>PESO</strong><br>${(shipment.weightGrams / 1000).toFixed(3)} kg</span><span><strong>ESTAFETA</strong><br>${safe(officeCode)}</span></div><div class="fine">${shipment.supplementaryServices.length ? `Adicionales: ${shipment.supplementaryServices.map((x) => safe(x.name)).join(' · ')}` : 'Manipular según condiciones del servicio postal.'}</div><footer class="label-footer"><div class="fine">S10 UPU · ${safe(shipment.trackingNumber)} · Conservar para rastreo</div></footer></main>`
  printDocument(shipment.trackingNumber, `${labelStyles}${labelBody}`, '4in 6in')
}
...
export default function SalesPage({ session }) {
  ...
  const visibleDestinations = catalog.destinations.filter((item) => form.shipmentScope === 'national' ? item.isDomestic : !item.isDomestic)
  const availableServices = selectedDestination
    ? catalog.postalServices.filter((service) => catalog.serviceWeightLimits.some((limit) => limit.postalServiceId === service.id && limit.destinationZone === selectedDestination.zone))
    : []
  ...
  function validateCurrentStep() {
    ...
    if (step === 1 && (!form.recipientFirstName.trim() || !form.recipientFirstLastName.trim() || !form.recipientAddress.trim())) return 'Completa el primer nombre, primer apellido y dirección del destinatario.'
      ...
      <Grid size={{ xs: 12, lg: 8 }}>
        <SectionPanel title={stepMeta[step].title.toUpperCase()} icon={stepMeta[step].icon} sx={{ height: '100%' }}>
          ...
          {step === 1 && <Grid container spacing={2}>
            <Grid size={{ xs: 12, sm: 4, md: 3 }}><TextField select fullWidth label="Título" value={form.recipientTitle} onChange={(e) => setForm({ ...form, recipientTitle: e.target.value })}><MenuItem value="">Sin título</MenuItem>{senderTitles.map((title) => <MenuItem key={title} value={title}>{title}</MenuItem>)}</TextField></Grid>
            <Grid size={{ xs: 12, sm: 8, md: 5 }}><TextField fullWidth label="Primer nombre" value={form.recipientFirstName} onChange={(e) => setForm({ ...form, recipientFirstName: e.target.value })} required /></Grid>
            <Grid size={{ xs: 12, md: 4 }}><TextField fullWidth label="Segundo nombre" value={form.recipientMiddleName} onChange={(e) => setForm({ ...form, recipientMiddleName: e.target.value })} /></Grid>
            <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Primer apellido" value={form.recipientFirstLastName} onChange={(e) => setForm({ ...form, recipientFirstLastName: e.target.value })} required /></Grid>
            <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Segundo apellido" value={form.recipientSecondLastName} onChange={(e) => setForm({ ...form, recipientSecondLastName: e.target.value })} /></Grid>
            <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Teléfono principal" value={form.recipientPhone} helperText="Nacional: 0000-0000 · Internacional: +00 000 0000" onChange={(e) => setForm({ ...form, recipientPhone: e.target.value })} /></Grid>
            <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Teléfono secundario" value={form.recipientSecondaryPhone} onChange={(e) => setForm({ ...form, recipientSecondaryPhone: e.target.value })} /></Grid>
            <Grid size={{ xs: 12, md: 8 }}><TextField fullWidth type="email" label="Correo electrónico" value={form.recipientEmail} onChange={(e) => setForm({ ...form, recipientEmail: e.target.value })} /></Grid>
            <Grid size={{ xs: 12, md: 4 }}><TextField fullWidth label="Fax" value={form.recipientFax} onChange={(e) => setForm({ ...form, recipientFax: e.target.value })} /></Grid>
            <Grid size={{ xs: 12, md: 4 }}><TextField fullWidth label="Provincia / estado" value={form.recipientProvince} onChange={(e) => setForm({ ...form, recipientProvince: e.target.value })} /></Grid>
            <Grid size={{ xs: 12, md: 4 }}><TextField fullWidth label="Ciudad" value={form.recipientCity} onChange={(e) => setForm({ ...form, recipientCity: e.target.value })} /></Grid>
            <Grid size={{ xs: 12, md: 4 }}><TextField fullWidth label="Código postal" value={form.recipientPostalCode} onChange={(e) => setForm({ ...form, recipientPostalCode: e.target.value })} /></Grid>
            <Grid size={{ xs: 12, md: 8 }}><TextField fullWidth label="Calle" value={form.recipientStreet} onChange={(e) => setForm({ ...form, recipientStreet: e.target.value })} /></Grid>
            <Grid size={{ xs: 12, md: 4 }}><TextField fullWidth label="Número de casa" value={form.recipientHouseNumber} onChange={(e) => setForm({ ...form, recipientHouseNumber: e.target.value })} /></Grid>
            <Grid size={12}><TextField fullWidth multiline minRows={3} label="Dirección completa del destinatario" value={form.recipientAddress} onChange={(e) => setForm({ ...form, recipientAddress: e.target.value })} required /></Grid>
          </Grid>}
          {step === 2 && <Grid container spacing={2}>
            <Grid size={{ xs: 12, md: 4 }}><TextField select fullWidth label="Tipo de destino" value={form.shipmentScope} onChange={(e) => { setForm({ ...form, shipmentScope: e.target.value, destinationId: '', postalServiceId: '', supplementaryServiceIds: [] }); setQuote(null) }}><MenuItem value="national">Nacional · Panamá</MenuItem><MenuItem value="international">Internacional</MenuItem></TextField></Grid>
            <Grid size={{ xs: 12, md: 8 }}><TextField select fullWidth label={form.shipmentScope === 'national' ? 'Provincia o destino nacional' : 'País internacional'} value={form.destinationId} onChange={(e) => { setForm({ ...form, destinationId: e.target.value, postalServiceId: '', supplementaryServiceIds: [] }); setQuote(null) }} required>{visibleDestinations.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</TextField></Grid>
            <Grid size={{ xs: 12, md: 7 }}><TextField select fullWidth label="Servicio postal disponible" value={form.postalServiceId} disabled={!selectedDestination} helperText={selectedDestination ? `${availableServices.length} servicio(s) con tarifa para este destino` : 'Primero selecciona el destino'} onChange={(e) => { setForm({ ...form, postalServiceId: e.target.value, supplementaryServiceIds: [] }); setQuote(null) }} required>{availableServices.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</TextField></Grid>
            <Grid size={{ xs: 12, md: 5 }}><TextField fullWidth type="number" label="Peso (kg)" value={form.weightKg} helperText={currentMaximumWeight ? `Formato 0.000 · máximo ${(currentMaximumWeight / 1000).toFixed(3)} kg` : 'Formato requerido: 0.000 kg'} inputProps={{ min: 0.001, max: currentMaximumWeight ? currentMaximumWeight / 1000 : undefined, step: 0.001 }} onBlur={() => Number(form.weightKg) > 0 && setForm((value) => ({ ...value, weightKg: Number(value.weightKg).toFixed(3) }))} onChange={(e) => { setForm({ ...form, weightKg: e.target.value, supplementaryServiceIds: [] }); setQuote(null) }} required /></Grid>
            <Grid size={12}><Alert severity="info">Los destinos de Panamá solo muestran servicios nacionales; los países extranjeros solo muestran servicios internacionales con tarifa vigente.</Alert></Grid>
          </Grid>}
