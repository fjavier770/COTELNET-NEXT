...
const initialForm = { senderName: '', senderDocument: '', senderPhone: '', senderEmail: '', senderAddress: '', recipientName: '', recipientPhone: '', recipientAddress: '', postalServiceId: '', destinationId: '', weightGrams: '', supplementaryServiceIds: [], paymentMethodId: '' }
...
export default function SalesPage({ session }) {
  ...
  function validateCurrentStep() {
    ...
    if (step === 2 && (!form.postalServiceId || !form.destinationId || Number(form.weightGrams) <= 0)) return 'Selecciona el servicio, destino e indica un peso válido.'
    return ''
  }

  async function calculateQuote() {
    const result = await apiRequest('/sales/quote', session.accessToken, { method: 'POST', body: JSON.stringify({ postalServiceId: Number(form.postalServiceId), destinationId: Number(form.destinationId), weightGrams: Number(form.weightGrams), supplementaryServiceIds: form.supplementaryServiceIds }) })
  ...
  async function finish() {
    try {
      const result = await apiRequest('/sales/shipments', session.accessToken, { method: 'POST', body: JSON.stringify({ ...form, postalServiceId: Number(form.postalServiceId), destinationId: Number(form.destinationId), weightGrams: Number(form.weightGrams), paymentMethodId: Number(form.paymentMethodId) }) })
