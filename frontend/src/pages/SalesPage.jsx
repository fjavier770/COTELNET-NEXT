...
function _printLegacyLabel(sale) {
  const shipment = sale.shipment
  const officeCode = sale.receipt?.officeCode || 'N/D'
  const countryCode = shipment.trackingNumber.slice(-2)
  const s10Styles = '<style>.label .s10-card{border-bottom:1px solid #b8c8d8;padding:2px 0 8px}.label .s10-head{display:flex;align-items:center;gap:7px}.label .s10-mark{font:900 26px/1 Arial,sans-serif;color:#111}.label .s10-brand{font:900 10px/1.05 Arial,sans-serif;color:#003b70}.label .s10-brand span{display:block;color:#ef7d00;font-size:7px;letter-spacing:.7px;margin-top:2px}.label .s10-meta{display:flex;align-items:center;gap:9px;margin-left:auto;font-size:9px;color:#526579}.label .s10-meta strong{color:#003b70;font-size:14px;letter-spacing:.5px}.label .s10-barcode{margin-top:5px}.label .s10-barcode svg{display:block;width:100%;height:auto;max-height:.82in}</style>'
  printDocument(shipment.trackingNumber, `${s10Styles}<main class="label"><header class="label-header"><div class="brand-lockup"><span class="brand-icon">✉</span><div><strong>CORREOS</strong><b>PANAMÁ</b></div></div><div class="format"><strong>CN-04</strong>ETIQUETA POSTAL<br>S10 · UPU</div></header><section class="s10-card"><div class="s10-head"><div class="s10-mark">R</div><div class="s10-brand">CORREOS<span>PANAMÁ</span></div><div class="s10-meta"><span>Estafeta ${safe(officeCode)} - ${safe(countryCode)}</span><strong>CN-04</strong></div></div><div class="s10-barcode">${shipment.barcodeSvg}</div></section><section class="address recipient"><div class="caption">DESTINATARIO / PARA</div><h2>${safe(shipment.recipientName)}</h2><p>${safe(shipment.recipientAddress)}<br><strong>${safe(shipment.destination)}</strong>${shipment.recipientPhone ? `<br>Tel. ${safe(shipment.recipientPhone)}` : ''}</p></section><section class="address"><div class="caption">REMITENTE / DE</div><h2>${safe(shipment.senderName)}</h2><p>${safe(shipment.senderAddress)}${shipment.senderPhone ? `<br>Tel. ${safe(shipment.senderPhone)}` : ''}</p></section><div class="service-strip"><span><strong>SERVICIO</strong><br>${safe(shipment.postalService)}</span><span><strong>PESO</strong><br>${(shipment.weightGrams / 1000).toFixed(3)} kg</span><span><strong>ESTAFETA</strong><br>${safe(officeCode)}</span></div><div class="fine">${shipment.supplementaryServices.length ? `Adicionales: ${shipment.supplementaryServices.map((x) => safe(x.name)).join(' · ')}` : 'Manipular según condiciones del servicio postal.'}</div><footer class="label-footer"><div class="fine">S10 UPU · ${safe(shipment.trackingNumber)} · Conservar para rastreo</div></footer></main>`, '4in 6in')
}
