...
function printDocument(title, body, pageSize = 'auto') {
  ...
  if (!popup) return
  popup.document.write(`<!doctype html><html><head><title>${safe(title)}</title><style>@page{size:${pageSize};margin:5mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#10233a;margin:0}h1{color:#003b70;margin:0}h2{margin:8px 0}.muted{color:#607080}.row{display:flex;justify-content:space-between;gap:20px}.box{border:1px solid #ccd5df;border-radius:8px;padding:14px;margin:12px 0}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:8px;border-bottom:1px solid #ddd}.total{font-size:24px;font-weight:700;color:#003b70}.barcode svg{display:block;width:100%;height:auto}small{font-size:11px}.receipt{width:72mm;margin:0 auto;font-family:'Courier New',monospace;color:#000;font-size:10.5px;line-height:1.25}.receipt .center{text-align:center}.receipt .brand{font-family:Arial,sans-serif;font-size:19px;font-weight:900;letter-spacing:.4px}.receipt .brand span{color:#ef7d00}.receipt .rule{border-top:1px dashed #000;margin:7px 0}.receipt .section{text-align:center;font-weight:700;margin:6px 0}.receipt .line{display:flex;justify-content:space-between;gap:8px}.receipt .line span:first-child{flex:1}.receipt .detail{margin:4px 0 8px}.receipt table{font-size:10px}.receipt th,.receipt td{padding:2px 0;border:0;vertical-align:top}.receipt th:last-child,.receipt td:last-child{text-align:right}.receipt .grand{font-weight:700;font-size:12px}.receipt .barcode{margin:8px auto 0;width:64mm}</style></head><body>${body}<script>window.onload=()=>window.print()</script></body></html>`)
...
function printInvoice(sale) {
  ...
  printDocument(sale.invoiceNumber, `<main class="receipt"><header class="center"><div class="brand"><span>CORREOS</span> PANAMÁ</div><div>Dirección Nacional de Correos y Telégrafos</div><strong>RECIBO DE VENTA</strong></header><div class="rule"></div><div>NO. RECIBO: ${safe(sale.invoiceNumber)}</div><div>FECHA: ${safe(new Date(sale.createdAtUtc).toLocaleString('es-PA'))}</div><div>CAJA: ${safe(receipt.terminalCode)}</div><div>OFICINISTA: ${safe(receipt.cashier)}</div><div>ESTAFETA: ${safe(receipt.officeCode)} - ${safe(receipt.officeName)}</div><div class="rule"></div><div class="section">DATOS DEL CLIENTE / REMITENTE</div><div>NOMBRE: ${safe(shipment.senderName)}</div><div>CÉDULA: ${safe(shipment.senderDocument || 'N/D')}</div><div>DIRECCIÓN: ${safe(shipment.senderAddress)}</div><div>TEL.: ${safe(shipment.senderPhone || 'N/D')}</div><div class="rule"></div><div class="section">DETALLE</div><div class="detail"><strong>DESTINATARIO:</strong> ${safe(shipment.recipientName)}<br><strong>DIRECCIÓN:</strong> ${safe(shipment.recipientAddress)}<br><strong>DESTINO:</strong> ${safe(shipment.destination)}<br><strong>SERVICIO:</strong> ${safe(shipment.postalService)}<br><strong>PESO:</strong> ${(shipment.weightGrams / 1000).toFixed(3)} kg<br><strong>CÓDIGO ENVÍO:</strong> ${safe(shipment.trackingNumber)}</div><table><thead><tr><th>CANT. DESCRIPCIÓN</th><th>IMPORTE</th></tr></thead><tbody>${sale.lines.map((line) => `<tr><td>${line.quantity} ${safe(line.description)}</td><td>B/.${Number(line.total).toFixed(2)}</td></tr>`).join('')}</tbody></table><div class="rule"></div><div class="line grand"><span>TOTAL:</span><strong>B/.${Number(sale.total).toFixed(2)}</strong></div>${sale.payments.map((payment) => `<div class="line"><span>${safe(payment.paymentMethod)}:</span><span>B/.${Number(payment.amount).toFixed(2)}</span></div>`).join('')}<div class="rule"></div><div class="center"><strong>DOCUMENTO NO FISCAL</strong><p>Para reclamos, presente este documento dentro de los 30 días siguientes a la fecha de emisión.</p><strong>TU CORREO SEGURO Y ECONÓMICO</strong><div class="barcode">${shipment.barcodeSvg}</div></div></main>`, '80mm auto')
}
